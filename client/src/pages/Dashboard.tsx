...
export function Dashboard() {
  ...
  const [budgetCategories, setBudgetCategories] = useState<Record<string, any[]>>({});
  ...
  const fetchCategoriesForBudgets = async () => {
    ...
    for (const budget of budgets) {
      try {
        ...
        console.error(`Failed to fetch categories for budget ${budget.id}:`, error);
      }
    }
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 tablet-padding">
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
            <div>
              <h1 className="responsive-heading gradient-text animate-fade-in">{t('dashboard.title')}</h1>
              <p className="responsive-text text-gray-600 mt-1">{t('dashboard.subtitle')}</p>
            </div>
            <LanguageSelector />
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <Button onClick={() => navigate("/create-budget")} className="gap-2">
              <Plus className="w-4 h-4" />
              {t('dialog.newBudget')}
            </Button>
            <Button variant="outline" onClick={() => navigate("/templates")} className="gap-2">
              <Layout className="w-4 h-4" />
              <span className="hidden sm:inline">{t('dashboard.templates')}</span>
            </Button>
            <Button variant="outline" onClick={() => navigate("/downloads")} className="gap-2">
        ...
        {budgets.length === 0 ? (
          <div className="bg-white rounded-lg border border-dashed border-gray-300 p-8 md:p-12 text-center">
            ...
            <p className="text-gray-500 text-sm">{t('dashboard.noBudgetsDesc')}</p>
            <Button className="mt-4 w-full sm:w-auto gap-2" onClick={() => navigate("/create-budget")}>
              <Plus className="w-4 h-4" />
        ...
        <footer className="mt-12 pt-8 border-t border-gray-200 text-center text-sm text-gray-600">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-4">
            <button onClick={() => navigate("/legal/privacy")} className="hover:text-primary transition-colors">
              {t('legal.privacy.title')}
            </button>
            <button onClick={() => navigate("/legal/terms")} className="hover:text-primary transition-colors">
              {t('legal.terms.title')}
            </button>
            <button onClick={() => navigate("/legal/cookies")} className="hover:text-primary transition-colors">
